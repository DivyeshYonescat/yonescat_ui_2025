            ...
            {token.user_type == "Merchant" ?
              <div className="flex flex-row items-center font-semibold text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden" >  
               <Image src={"/images/main/menu/dropdown/avatar-icon.svg"} className='mr-[10px]' alt='avatar-icon' width={20} height={20} />
                {token.username} </div> : 
              <div className="flex flex-row items-center font-semibold  rounded-md text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden" > 
                 <Image src={"/images/main/menu/dropdown/avatar-icon.svg"} className='mr-[10px]' alt='avatar-icon' width={20} height={20} />
                {token.username} </div>
            }
          </DropdownMenuItem>
