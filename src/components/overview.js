...
const Overview = ({list})=>{
  ...
  return (
    <>  
      <section className='overview-main block mt-[80px]'>
        <div className='container'>
          <div className='overview-inner content-container grid max-md:grid-cols-1 grid-cols-[45%_55%] ' ref={containerRef}>
            <div className='overview-img-section h-screen max-md:h-auto max-md:min-h-[300px] max-md:mt-[60px] relative'>
              {list.map((images) => <>
            ...
            <div className='overview-text-section max-md:flex max-md:w-[100%] ' ref={imagesWrapperRef}>
              {list.map((list) => <>
              <div key={`overview-text_${list.id}`} className='overview-text-main max-w-[100%] max-md:min-w-screen p-8 h-[calc(100vh-80px)] max-md:h-auto max-md:items-baseline flex items-center' data-marker-content={`image_${list.id}`}>
                <div>
